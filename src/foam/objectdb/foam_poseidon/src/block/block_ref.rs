 ...
 */
pub(crate) fn block_addr_unpack(block_handle: &BlockHandle, bytes: &[u8], addr_size: usize) -> FPResult<BlockRef> {
...
pub(crate) fn block_addr_pack(bh: &BlockHandle, bytes: &mut [u8], block_ref: &BlockRef) -> FPResult<()> {
    let mut offset = 0u64;
    let mut size = 0u64;
