...
impl fmt::Debug for RWBytes {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        ...
        write!(f, "RWBytes({bytes_truncated:?}, len={})", self.len())
    }
}

impl PartialEq for RWBytes {
    fn eq(&self, other: &RWBytes) -> bool {
...
impl PartialEq<[u8]> for RWBytes {
    fn eq(&self, other: &[u8]) -> bool {
        self.as_slice() == other
    }
}

impl PartialEq<str> for RWBytes {
    fn eq(&self, other: &str) -> bool {
        self.as_slice() == other.as_bytes()
    }
}

impl<'a, T: ?Sized> PartialEq<&'a T> for RWBytes
where RWBytes: PartialEq<T>
{
